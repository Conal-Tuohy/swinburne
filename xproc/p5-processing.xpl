...
<!-- for request URI "admin/purge", clear the Solr index -->
	...
	<p:declare-step name="xinclude-directory" type="chymistry:xinclude-directory">
		...
		During processing, periodically updates the status file referred to by $status-page-href to report on progress
		-->
		<p:input port="source"/>
		<p:output port="result"/>
		<p:option name="status-page-href" required="true"/>
		<p:option name="status-update-interval" required="true"/>
		<p:add-xml-base relative="false" all="true"/>
		<p:viewport name="file" match="c:file">
			...
			<!-- update status message every 25 files -->
			<p:choose>
				<p:when test="p:iteration-position() mod 25 = 0">
					<chymistry:xinclude-status-page title="XInclude processing is running">
						<p:with-option name="message" select="
							concat(
								'The XInclude process is running. Processing ',
								$file-name,
								'; file ',
						...
						"/>
						<p:with-option name="update-interval" select="$status-update-interval"/>
					</chymistry:xinclude-status-page>
					<p:store>
						<p:with-option name="href" select="$status-page-href"/>
					</p:store>
				</p:when>
				<p:otherwise>
					<p:sink/>
	...
	<p:declare-step name="xinclude-status-page" type="chymistry:xinclude-status-page">
		<p:output port="result"/>
		<p:option name="update-interval"/>
		<p:option name="title"/>
		<p:option name="message"/>
		<p:template>
			<p:with-param name="update-interval" select="$update-interval"/>
					...
					<c:response status="200">
						<c:body content-type="text/html">
							<html xmlns="http://www.w3.org/1999/xhtml">
								<head>
									<title>{$title}</title>
									<meta http-equiv="refresh" content="{$update-interval}"/>
								</head>
								<body>
									<h1>{$title}</h1>
									<p>{$message}</p>
		...
		<cx:message>
			<p:with-option name="message" select="concat('Updating XInclude status report: ', $message)"/>
		</cx:message>
	</p:declare-step>
	
	<p:declare-step name="xinclude" type="chymistry:xinclude">
		<p:input port="source"/>
		<!-- NB this step has no output port because it's intended to be run
		in the background; it reports on its progress by repeatedly updating
		a "status" page whose location is given in the option below -->
		<p:option name="status-page-href" required="true"/>
		<p:option name="status-update-interval" required="true"/>
		...
		<!-- store a response that announces that XInclude processing has started -->
		<chymistry:xinclude-status-page title="XInclude processing has started" message="Please wait.">
		...
		<chymistry:xinclude-directory>
			...
			<p:with-option name="status-page-href" select="$status-page-href"/>
		</chymistry:xinclude-directory>
					...
					<xsl:stylesheet version="3.0" 
						...
						<xsl:template match="/">
							...
							<xsl:variable name="title" select="
								if ($errors) then 
									'XInclude processing completed with errors' 
								else 
									'XInclude processing completed successfully'
							"/>
							<c:response status="200">
